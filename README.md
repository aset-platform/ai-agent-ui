...
- **Algo Trading** *(v1, 2026-05-09; v2, 2026-05-09; intraday clock, 2026-06-28; manual exit, 2026-07-01)* — pro/superuser route `/algo-trading` with backtest + paper + live trading on Indian equities. 8-tab strip: Connect Broker (Kite OAuth via frontend bounce page), Instruments (Kite-derived master, ~1.3M rows, NSE/BSE equity → `our_ticker`), Strategies (visual builder + JSON pane + **Levers panel** with auto-discovered tunables), Backtest (T+1 open-to-open fills, on-the-fly SMA + golden_cross_days_ago indicators, 3-tier `RiskEngine` gate, async background job, equity curve + trade table with column selector + CSV; **intraday execution clock**: daily strategies get ~25 exit checks/day via `stocks.intraday_bars` 15m data, replicating the live GTT/trailing loop — see [docs/algo-trading/intraday-execution-clock.md](docs/algo-trading/intraday-execution-clock.md)), Paper Trading (tick-driven runtime against live Kite WS), Live Trading (Kite LIMIT orders, GTT trailing stops, position sizing caps, Holdings tab with **manual exit button** — cancels GTT + places LIMIT SELL on demand during market hours), Performance (cohort comparison across modes), Replay (cross-mode event timeline with mode/type/strategy/date filters), Settings (kill switch with PG + Redis mirror). Storage: Postgres `algo.*` (mutable state) + Iceberg `algo.events` (canonical event log) + Iceberg `algo.intraday_bars` (resampled bars). Secrets via macOS Keychain → docker-compose mount → `load_secret()` (mirrors prod CSI Secrets-Store driver shape). Full docs: [docs/algo-trading/overview.md](docs/algo-trading/overview.md).
