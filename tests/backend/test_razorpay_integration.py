...
class TestFindUserByRazorpay:
    ...
    @pytest.mark.asyncio
    async def test_find_by_subscription_id(self):
        repo = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_find_by_customer_id_fallback(self):
        repo = AsyncMock()
        repo.list_all.return_value = [
            {
                "user_id": "u2",
    ...
    @pytest.mark.asyncio
    async def test_sub_id_prioritised_over_cust_id(
        self,
    ):
    ...
    async def test_not_found_returns_none(self):
        repo = AsyncMock()
        repo.list_all.return_value = []
        assert await _find_user_by_razorpay(
            repo, "sub_x", "cust_x",
        ) is None
...
class TestWebhookHandlers:
    ...
    @pytest.mark.asyncio
    async def test_handle_charged_activates_tier(
        self,
    ):
        mock_repo = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_handle_charged_ignores_stale_sub(
        self,
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_cancelled_resets_to_free(
        self,
    ):
        mock_repo = AsyncMock()
        mock_repo.list_all.return_value = [
        ...
        assert (
            updates["razorpay_subscription_id"] is None
        )

    @pytest.mark.asyncio
    async def test_handle_cancelled_ignores_stale_sub(
        self,
    ):
        mock_repo = AsyncMock()
        mock_repo.list_all.return_value = [
    ...
    @pytest.mark.asyncio
    async def test_handle_charged_no_user_no_crash(
        self,
    ):
        mock_repo = AsyncMock()
        mock_repo.list_all.return_value = []
    ...
    @pytest.mark.asyncio
    async def test_handle_charged_empty_id_skipped(
        self,
    ):
        mock_repo = AsyncMock()
        with patch(
    ...
    @pytest.mark.asyncio
    async def test_idempotent_charged(self):
        ...
        for ca in mock_repo.update.call_args_list:
            updates = ca[0][1]
            assert (
