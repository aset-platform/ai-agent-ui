...
class TestCreateAccessToken:
    ...
    def test_returns_non_empty_string(self):
        ...
        token = create_access_token(
            user_id="u1", email="u@test.com", role="general",
            secret_key=_SECRET, expire_minutes=15,
        )
    ...
    def test_payload_contains_expected_claims(self):
        ...
        payload = decode_token(token, _SECRET, _fresh_deny_list(), expected_type="access")
...
class TestCreateRefreshToken:
    ...
    def test_returns_string(self):
        ...
        token = create_refresh_token(
            user_id="u1", secret_key=_SECRET, expire_days=7,
        )
        assert isinstance(token, str)
    ...
    def test_type_is_refresh(self):
        ...
        payload = decode_token(token, _SECRET, _fresh_deny_list(), expected_type="refresh")
...
class TestDecodeToken:
    ...
    def test_wrong_secret_raises(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            decode_token(token, "wrong-secret", _fresh_deny_list(), expected_type="access")
    ...
    def test_wrong_type_raises(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            decode_token(token, _SECRET, _fresh_deny_list(), expected_type="refresh")
        assert exc_info.value.status_code == 401
    ...
    def test_expired_token_raises(self):
        ...
        token = create_access_token(
            user_id="u1", email="u@test.com", role="general",
            secret_key=_SECRET, expire_minutes=-1,  # already expired
        )
        with pytest.raises(HTTPException) as exc_info:
            decode_token(token, _SECRET, _fresh_deny_list(), expected_type="access")
        assert exc_info.value.status_code == 401
...
class TestRevokeAndDenyList:
    ...
    def test_revoked_token_is_revoked(self):
        from auth.tokens import create_refresh_token, is_token_revoked, revoke_refresh_token
        deny_list = _fresh_deny_list()
        token = create_refresh_token(
            user_id="u1", secret_key=_SECRET, expire_days=7,
        )
    ...
    def test_decode_revoked_token_raises(self):
        from auth.tokens import create_refresh_token, decode_token, revoke_refresh_token
        from fastapi import HTTPException
        deny_list = _fresh_deny_list()
        token = create_refresh_token(
            user_id="u1", secret_key=_SECRET, expire_days=7,
