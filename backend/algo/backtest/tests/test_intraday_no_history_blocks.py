...
def test_5m_strategy_blocks_when_no_5m_history():
    strategy = parse_strategy(_v5_strategy())
    req = BacktestRequest(
        strategy_id=strategy.id, period_start=_BASE,
        period_end=_BASE + timedelta(days=5), interval_sec=300,
    )
    # only 15m exists for the ticker
    cov = {"FAKE.NS": TickerCoverage(
        "FAKE.NS", 900, _BASE, _BASE + timedelta(days=5), 5)}
    with patch(
        ...
        with pytest.raises(ValueError, match="No 5m history"):
            run_backtest(
                strategy=strategy, request=req,
                user_id=uuid4(), universe=["FAKE.NS"],
            )
...
def test_5m_not_blocked_when_5m_coverage_exists():
    strategy = parse_strategy(_v5_strategy())
    req = BacktestRequest(
        strategy_id=strategy.id, period_start=_BASE,
        period_end=_BASE + timedelta(days=5), interval_sec=300,
    )
    # 5m coverage exists for the ticker
    cov = {"FAKE.NS": TickerCoverage(
        "FAKE.NS", 300, _BASE, _BASE + timedelta(days=5), 5)}
    with patch(
        "backend.algo.backtest.runner.intraday_coverage", return_value=cov,
    ), patch("backend.algo.backtest.runner.flush_events"):
        try:
            ...
            assert "history" not in str(e), (
                "5m with 5m coverage must not be no-history-blocked"
            )
        except Exception:
            pass  # other downstream errors are fine
