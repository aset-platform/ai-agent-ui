...
- Use `pressSequentially()` not `fill()` for React 19 controlled inputs (textarea/input with `value={state}` + `onChange`).
- `dbc.*` components (dash_bootstrap_components 2.0.4) do NOT accept `data-testid` — wrap in `html.Div(**{"data-testid": "..."})`.
- Keep Playwright `outputDir` outside the project tree (`/tmp/`) to avoid triggering the Dash debug reloader.
- Use `{ force: true }` for clicks blocked by the Dash debug toolbar overlay.
