...
10. No bare `print()` — `_logger = logging.getLogger(__name__)`. Levels: DEBUG/INFO/WARNING/ERROR. Never log secrets/tokens/passwords at INFO. **Caught exceptions in long-running jobs/pipelines MUST log with `exc_info=True`** — without it the traceback is lost and prod failures are blind. 2026-05-12 USA pipeline failure cost 15 min of reverse-engineering before this was added.
...
20. **NEVER delete Iceberg metadata/parquet directly** — use `overwrite()` / `delete_rows()` API. Direct `rm` breaks SQLite catalog. The sanctioned reclamation path is `cleanup_orphans_v2()` (PyIceberg 0.11.1 native expire_snapshots + reference-set guard) — `→ iceberg-orphan-sweep-design`.
    - **Enroll write-heavy tables in BOTH lists**: `_HOT_ICEBERG_TABLES` (`backend/jobs/executor.py`) AND `ALL_TABLES` (`backend/maintenance/iceberg_maintenance.py`) — in the SAME PR as the schema creation. Skipping is silent; the table just bloats forever (2026-05-12: `algo.events` hit 11 GB of metadata after never being maintained). `→ iceberg-maintenance-enrollment`
...
- **PyArrow `pa.string()` rejects NaN** with the un-googleable error "Expected bytes, got a 'float' object". Sanitise ticker/date columns at every `pa.table(...)` boundary — drop rows where the string field is NaN/empty before the cast. yfinance bulk-download + per-ticker retry merge is the usual source.
...
- **`cleanup_orphans_v2(dry_run=True)` is misleading** — `dry_run` only gates file *deletion*; snapshot *expiry* commits regardless. Run with `skip_backup=False` (the default) so the pre-op backup catches you if expiry has unintended effects. First run on a never-maintained table takes 5-15 min (mass snapshot expiry commit); subsequent runs ~20s.
...
| Add a swing-setup regime / tune filter thresholds | 5.4, 5.13 | `swing-setups-design` |
...
| Add a new Iceberg table | 4.3 | `db-table-inventory`, `iceberg-maintenance`, `iceberg-maintenance-enrollment` |
...
| Add a pipeline / chain step | 5.1 | `pipeline-chaining-dag`, `iceberg-daily-pipeline-compaction`, `pipeline-quality-assertions` |
| Add data-quality assertions to a pipeline step | 5.1 | `pipeline-quality-assertions` |
...
| Debug Iceberg metadata bloat / disk-usage spike | 4.3 #20 | `iceberg-maintenance-enrollment`, `iceberg-orphan-sweep-design` |
| Debug silent-success pipeline run (status=ok but output dirty) | 5.1 | `pipeline-quality-assertions` |

All paths are `shared/<category>/<name>`. `list_memories` browses ~170 memories (~71 architecture · 24 conventions · 37 debugging · 3 onboarding · 26 dated session checkpoints).
